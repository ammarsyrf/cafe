        ...
        do {
            ...
            if (isset($_FILES['image']) && $_FILES['image']['error'] == UPLOAD_ERR_OK) {
                try {
                    ...
                    Security::validateFileUpload($file, ['jpg', 'jpeg', 'png', 'webp'], 5242880); // 5MB limit
                    ...
                    $new_filename = Security::generateSecureFilename($file['name']);
                    $new_filename = 'menu_' . $new_filename;
                    ...
                    Security::logSecurityEvent('menu_image_uploaded', ['filename' => $new_filename, 'admin_id' => $_SESSION['admin_id']]);
                } catch (Exception $e) {
                    Security::logSecurityEvent('menu_upload_failed', ['error' => $e->getMessage(), 'admin_id' => $_SESSION['admin_id']]);
                    $response['message'] = 'Gagal mengupload gambar: ' . $e->getMessage();
                    break;
                }
            }
...
$total_menu = count($menu_items);
$out_of_stock = 0;
$low_stock = 0;
$very_low_stock = 0;
$menu_per_category = array_fill_keys($categories, 0);
foreach ($menu_items as $item) {
    if ($item['stock'] <= 0) $out_of_stock++;
    if ($item['stock'] <= 10) $low_stock++;
    if ($item['stock'] <= 5) $very_low_stock++;
    if (isset($menu_per_category[$item['category']])) $menu_per_category[$item['category']]++;
}
...
<div class="container mx-auto p-4 md:p-6 lg:p-8">
    ...
    <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-5 gap-6 mb-8">
        <div class="bg-white rounded-xl shadow-md p-6 flex flex-col items-center">
            <div class="text-2xl font-bold text-blue-600 mb-2">Kategori</div>
            <ul class="text-gray-700 text-sm">
                <?php foreach ($categories as $cat): ?>
                    <li><?= htmlspecialchars($cat) ?> <span class="text-xs text-gray-500">(<?= $menu_per_category[$cat] ?> menu)</span></li>
                <?php endforeach; ?>
            </ul>
        </div>
        <div class="bg-white rounded-xl shadow-md p-6 flex flex-col items-center">
            <div class="text-2xl font-bold text-green-600 mb-2">Total Menu</div>
            <div class="text-4xl font-extrabold"><?= $total_menu ?></div>
        </div>
        <div class="bg-white rounded-xl shadow-md p-6 flex flex-col items-center">
            <div class="text-2xl font-bold text-yellow-600 mb-2">Menu Tampil</div>
            <div class="text-4xl font-extrabold" id="displayedMenuCount"><?= $total_menu ?></div>
        </div>
        <div class="bg-white rounded-xl shadow-md p-6 flex flex-col items-center">
            <div class="text-2xl font-bold text-red-600 mb-2">Menu Habis Stok</div>
            <div class="text-4xl font-extrabold"><?= $out_of_stock ?></div>
        </div>
        <div class="bg-white rounded-xl shadow-md p-6 flex flex-col items-center">
            <div class="text-2xl font-bold text-orange-500 mb-2">Menu Stok ≤ 10</div>
            <div class="text-4xl font-extrabold"><?= $low_stock ?></div>
            <?php if ($very_low_stock > 0): ?>
                <div class="mt-2 text-sm text-red-600 font-bold flex items-center gap-2">
                    <i class="fas fa-exclamation-triangle"></i>
                    Ada <?= $very_low_stock ?> menu stok sangat sedikit (≤ 5)!
                </div>
            <?php endif; ?>
        </div>
    </div>
    <script>
        // Update displayed menu count on search
        document.addEventListener('DOMContentLoaded', function() {
            const searchInput = document.getElementById('searchInput');
            const displayedMenuCount = document.getElementById('displayedMenuCount');
            ...
            function updateCount() {
                ...
                displayedMenuCount.textContent = count;
            }
            searchInput.addEventListener('keyup', updateCount);
            updateCount();
        });
    </script>
            ...
            <?php foreach ($menu_items as $item) : ?>
                <div class="menu-card bg-white rounded-xl shadow-lg overflow-hidden flex flex-col transition-transform transform hover:-translate-y-2" data-id="<?= $item['id'] ?>" data-name="<?= strtolower(htmlspecialchars($item['name'])) ?>" data-category="<?= strtolower(htmlspecialchars($item['category'])) ?>">
                    ...
                    <div class="p-5 flex-grow">
                        ...
                        <p class="stock-text text-sm font-medium mt-2 <?= $item['stock'] <= 5 ? 'text-red-600' : ($item['stock'] <= 10 ? 'text-yellow-600' : 'text-gray-700') ?>">Stok: <?= $item['stock'] ?></p>
                    </div>
    ...
    document.addEventListener('DOMContentLoaded', function() {
        ...
        addonListContainer.addEventListener('submit', async e => {
            if (e.target.classList.contains('add-option-form')) {
                ...
                if (!data.name) return showAjaxNotification('Nama opsi tidak boleh kosong.', 'error');
