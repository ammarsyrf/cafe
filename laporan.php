...
<body class="bg-gray-50">
    ...
    <main class="container mx-auto px-4 py-6 sm:py-8 max-w-lg">
        <!-- Notifikasi -->
        ...
        <div id="tab-content">
            ...
            <div id="riwayat-content" class="tab-pane hidden space-y-3 sm:space-y-4">
                <?php if (!empty($order_history)): ?>
                    <?php foreach ($order_history as $order): ?>
                        <div class="bg-white rounded-xl shadow-sm overflow-hidden">
                            <div class="p-4 cursor-pointer flex justify-between items-center" onclick="toggleOrderDetails(<?= $order['id']; ?>)">
                                <div class="flex-1 pr-4">
                                    <p class="font-bold text-gray-800 truncate">Pesanan #<?= htmlspecialchars($order['id']); ?></p>
                                    <p class="text-sm text-gray-500"><?= date('d M Y, H:i', strtotime($order['order_date'])); ?></p>
                                </div>
                                <div class="text-right flex-shrink-0">
                                    <p class="font-bold text-base sm:text-lg text-gray-900">Rp <?= number_format($order['total_amount'] ?? 0); ?></p>
                                ...
                                <i id="icon-<?= $order['id']; ?>" class="fas fa-chevron-down text-gray-400 transition-transform duration-300 ml-4"></i>
                            </div>
                            <div id="details-<?= $order['id']; ?>" class="hidden px-4 pb-4 border-t border-gray-100">
                                <div class="pt-4">
                                    ...
                                    <div class="space-y-1 text-sm text-gray-600">
                                        ...
                                        <?php if ($order['discount_amount'] > 0): ?>
                                            <div class="flex justify-between text-red-600">
                                                <span>Diskon</span>
                                                <span class="font-medium">- Rp <?= number_format($order['discount_amount']); ?></span>
                                            </div>
                                        <?php endif; ?>
                                        <div class="flex justify-between">
                                            <span>PPN (11%)</span>
