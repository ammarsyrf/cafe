...
if ($_SERVER['REQUEST_METHOD'] == 'POST' && isset($_POST['login'])) {
        ...
        $error_message = 'Username dan password harus diisi.';
                ...
                try {
                    ...
                    if ($result->num_rows > 0) {
                        ...
                        if (!$user['is_active']) {
                            Security::logSecurityEvent('admin_login_inactive_account', ['username' => $username]);
                            $error_message = 'Akun Anda tidak aktif. Hubungi administrator.';
                        }
                        // Verifikasi password
                        else if (Security::verifyPassword($password, $user['password'])) {
                            // Reset rate limit on successful login
                            Security::resetRateLimit($clientIdentifier);

                            // Regenerate session ID untuk keamanan
                            session_regenerate_id(true);

                            // Set session admin dengan data yang aman
                            $_SESSION['admin_logged_in'] = true;
                            $_SESSION['admin_id'] = $user['id'];
                            $_SESSION['admin_username'] = Security::sanitizeInput($user['username']);
                            $_SESSION['admin_name'] = Security::sanitizeInput($user['name']);
                            $_SESSION['admin_email'] = Security::sanitizeInput($user['email']);
                            $_SESSION['admin_role'] = $user['role'];
                            $_SESSION['admin_login_time'] = time();

                            Security::logSecurityEvent('successful_admin_login', ['admin_id' => $user['id'], 'role' => $user['role']]);
                            Security::safeRedirect('superadmin/index.php');
                        } else {
                            // Increment failed attempts
                            Security::incrementRateLimit($clientIdentifier);
                            Security::logSecurityEvent('failed_admin_login', ['username' => $username, 'reason' => 'invalid_password']);
                            $error_message = 'Username atau password salah.';
                        }
                    } else {
                        // Increment failed attempts
                        Security::incrementRateLimit($clientIdentifier);
                        Security::logSecurityEvent('failed_admin_login', ['username' => $username, 'reason' => 'user_not_found']);
                        $error_message = 'Username atau password salah.';
                    }
                    $stmt->close();
                } catch (Exception $e) {
                    Security::logSecurityEvent('admin_login_database_error', ['error' => $e->getMessage()]);
    ...
    if (!empty($error_message)) {
        ...
        Security::safeRedirect('admin_login.php?error=1');
    }
}
